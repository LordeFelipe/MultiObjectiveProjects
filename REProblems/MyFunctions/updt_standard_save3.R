updt_standard_save3 <- function(X, Xt, Y, Yt, V, Vt, sel.indx, B, ...){
  ...
  Yaux = matrix(c(Ynext[,1], Ynext[,2], Ynext[,3],ifelse(Vnext$v>0,0,1)), nrow = 325, ncol = 4)
  write(t(Yaux),file = paste(getwd(), sprintf("MyArchive%d.txt",i), sep="/"), ncolumns = 4, sep = " ", append = TRUE)
