CRE2_hypervolume_files <- function(case, filename, n_individuals, n_objectives, n_iterations...){
  ...
  maxob1 = max(A[which(A[,3,1,1]==1),1,1,1])
  minob1 = min(A[which(A[,3,1,1]==1),1,1,1])
  maxob2 = max(A[which(A[,3,1,1]==1),2,1,1])
  minob2 = min(A[which(A[,3,1,1]==1),2,1,1])
  for(i in 1:20){
    for(j in 1:length(filename)){
      maxob1 = ifelse(maxob1 > max(A[which(A[,3,i,j]==1),1,i,j]), maxob1, max(A[which(A[,3,i,j]==1),1,i,j]))
      minob1 = ifelse(minob1 < min(A[which(A[,3,i,j]==1),1,i,j]), minob1, min(A[which(A[,3,i,j]==1),1,i,j]))
      
      maxob2 = ifelse(maxob2 > max(A[which(A[,3,i,j]==1),2,i,j]), maxob2, max(A[which(A[,3,i,j]==1),2,i,j]))
      minob2 = ifelse(minob2 < min(A[which(A[,3,i,j]==1),2,i,j]), minob2, min(A[which(A[,3,i,j]==1),2,i,j]))
    }
  }
  for (ite in 1:20){
    ...
    for(j in 1:n_iterations){
      ...
      #Only one feasible solution
        ...
        NewHyper[ite,j] = dominated_hypervolume(matrix(Newnormalized[which(Newnormalized[,3,j] == 1),1:2,j]), (c(1.1,1.1)))
      }
      #Multiple feasible solutions
      else{
        NewHyper[ite,j] = dominated_hypervolume(t(Newnormalized[which(Newnormalized[,3,j] == 1),1:2,j]), (c(1.1,1.1)))
      }
    } 
  }
  return(NewHyper)
