...
int main(int argc, char **argv) {
  ...
  FILE* fp = fopen("CREProblems/CRE22/pop_vars_eval.txt","r");
  FILE* out = fopen("CREProblems/CRE22/pop_vars_obj.txt","w");
  FILE* cons = fopen("CREProblems/CRE22/pop_vars_cons.txt","w");

  double aux;
  i = 0;
  while(fscanf(fp, "%lf", &aux) != EOF){
      x[i] = aux;
      i++;
